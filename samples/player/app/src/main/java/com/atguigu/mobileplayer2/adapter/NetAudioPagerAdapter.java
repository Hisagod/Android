...
//                viewHolder.jcv_videoplayer.setUp(mediaItem.getVideo().getVideo().get(0), mediaItem.getVideo().getThumbnail().get(0), null);
            ...
            case TYPE_IMAGE://图片
                ...
                int  height = mediaItem.getImage().getHeight()<= ScreenUtils.getScreenHeight()*0.75?mediaItem.getImage().getHeight(): (int) (ScreenUtils.getScreenHeight() * 0.75);

                LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(ScreenUtils.getScreenWidth(),height);
